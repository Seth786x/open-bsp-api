...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    if (method === 'GET') {
      ...
      const { data, error } = await adminClient.from("webhooks").select("*").eq("organization_id", organization_id);
    ...
    if (method === 'POST') {
      ...
      if (!organization_id || !url) {
        return new Response(JSON.stringify({
          error: "Validation Failed: Missing url or org ID"
        }), {
          status: 400,
          headers: {
            ...corsHeaders,
    ...
    if (method === 'DELETE') {
      ...
      const { error } = await adminClient.from("webhooks").delete().eq("id", webhook_id);
      if (error) throw error;
      return new Response(JSON.stringify({
        success: true
      }), {
