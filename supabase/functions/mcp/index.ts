...
app.use("*", async (c, next)=>{
  try {
    ...
    const { data: apiKey, error: apiKeyError } = await supabase.from("api_keys").select("organization_id").eq("key", token).maybeSingle();
 ...
 */ function createMcpServer(supabase, orgId, allowedContacts, allowedAccounts) {
  ...
  server.registerTool("fetch_conversation", {
  ...
  server.registerTool("fetch_template", {
