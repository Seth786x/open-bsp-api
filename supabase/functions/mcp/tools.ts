...
import { fetchTemplate as fetchTemplateMethod, listTemplates as listTemplatesMethod } from "../whatsapp-management/templates.ts";
 ...
 */ async function resolveAccount(params) {
  ...
  let query = params.supabase.from("organizations_addresses").select("address, phone:extra->>phone_number, name:extra->>verified_name").eq("organization_id", params.orgId).eq("service", "whatsapp").eq("status", "connected");
  ...
  if (accounts.length > 1) {
    throw new Error(`Multiple accounts found. Please specify account_phone. Available accounts: ${availablePhones}`);
  }
  return accounts[0];
}
export async function listConversations(params) {
  ...
  let recentQuery = params.supabase.from("messages").select("conversation_id").eq("organization_id", params.orgId).eq("organization_address", account.address).eq("service", "whatsapp").neq("direction", "internal").order("timestamp", {
    ascending: false
  ...
  const { data: conversations } = await params.supabase.from("conversations").select(`
      *,
      messages(*),
      contact_address:contacts_addresses(*, contact:contacts(*))
    `).in("id", conversationIds).eq("status", "active").neq("messages.direction", "internal").order("timestamp", {
  ...
  const idOrder = new Map(conversationIds.map((id, i)=>[
      id,
      i
    ]));
  const sortedConversations = conversations.sort((a, b)=>(idOrder.get(a.id) ?? 999) - (idOrder.get(b.id) ?? 999));
  return {
    account: {
      name: account.name,
      phone: account.phone
...
export async function fetchConversation(params) {
  ...
  const { data: conversation } = await params.supabase.from("conversations").select(`
      *,
      messages(direction, content, timestamp, status),
      contacts_addresses(*, contacts(*))
    `).eq("organization_id", params.orgId).eq("contact_address", contactPhone).eq("organization_address", account.address).eq("service", "whatsapp").eq("status", "active").neq("messages.direction", "internal").order("created_at", {
    ascending: false
  }).order("timestamp", {
    ascending: false,
    referencedTable: "messages"
  }).limit(1).limit(params.limit || 10, {
    referencedTable: "messages"
  ...
  return {
    ...
    contact: {
      name: conversation.contacts_addresses?.contacts?.name || conversation.contacts_addresses?.extra?.name,
      phone: contactPhone
    },
...
export async function searchContacts(params) {
  ...
  const allowedContacts = params.allowedContacts;
  const select = params.name ? "phone:address, contact:contacts!inner(name)" : "phone:address, contact:contacts(name)";
  let query = params.supabase.from("contacts_addresses").select(select).eq("organization_id", params.orgId).eq("service", "whatsapp").eq("status", "active");
  if (allowedContacts.length) {
...
export async function listAccounts(params) {
  const allowedAccounts = params.allowedAccounts;
  let query = params.supabase.from("organizations_addresses").select("phone:extra->>phone_number, name:extra->>verified_name").eq("organization_id", params.orgId).eq("service", "whatsapp").eq("status", "connected");
  if (allowedAccounts.length) {
    query = query.in("extra->>phone_number", allowedAccounts);
  }
  const { data: accounts } = await query.throwOnError();
...
export async function sendMessage(params) {
  ...
  if (params.content.kind !== "template") {
    const { data: lastMsg } = await params.supabase.from("messages").select("timestamp").eq("organization_id", params.orgId).eq("organization_address", account.address).eq("contact_address", contactPhone).eq("direction", "incoming").order("timestamp", {
      ascending: false
    }).limit(1).maybeSingle();
    if (!lastMsg) {
  ...
  const isSupported = params.content.type === "text" || params.content.type === "data" && params.content.kind === "template";
  if (!isSupported) {
...
export async function fetchTemplate(params) {
  ...
  const t = await fetchTemplateMethod(params.supabase, params.orgId, account.address, {
