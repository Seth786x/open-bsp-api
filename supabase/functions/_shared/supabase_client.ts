...
export function createClient(req) {
  ...
  return createClientBase(Deno.env.get("SUPABASE_URL"), Deno.env.get("SUPABASE_ANON_KEY"), {
    auth: {
...
export function createUnsecureClient() {
  ...
  return createClientBase(Deno.env.get("SUPABASE_URL"), Deno.env.get("SUPABASE_SERVICE_ROLE_KEY"), {
    auth: {
      persistSession: false
    }
  });
