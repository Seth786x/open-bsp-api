...
export const GetDbSchemaInputSchema = z.object({
  schemas: z.array(z.string()).optional().describe("Optional: schema names to include.")
});
...
function getTablesQuery(quotedSchemas) {
  return `
    SELECT
...
function getColumnsQuery(quotedSchemas, driver) {
  let typeColumn = "";
  if (driver === "postgres") {
    typeColumn = "udt_name";
...
function getPostgresConstraintsQuery(quotedSchemas) {
  return `
    SELECT
      tc.table_schema,
      tc.table_name,
...
function getMySQLConstraintsQuery(_quotedSchemas) {
  return `
    SELECT
      tc.table_schema,
      tc.table_name,
...
function getPostgresTableCommentsQuery(quotedSchemas) {
  return `
      SELECT
        n.nspname AS schema,
        c.relname AS name,
...
function getMySQLTableCommentsQuery(_quotedSchemas) {
  return `
      SELECT
        t.table_schema AS schema,
        t.table_name AS name,
...
function getPostgresColumnCommentsQuery(quotedSchemas) {
  return `
      SELECT
...
function getMySQLColumnCommentsQuery(_quotedSchemas) {
  return `
      SELECT
        c.table_schema,
        c.table_name,
...
function getPostgresEnumsQuery(quotedSchemas) {
  return `
      SELECT
        n.nspname AS schema,
        t.typname AS name,
...
class BaseClient {
  ...
  async tables() {
    return await this.execute(getTablesQuery(this.quotedSchemas));
  }
  async columns() {
    return await this.execute(getColumnsQuery(this.quotedSchemas, this.driver));
  }
  constraints() {
    return Promise.reject(new Error("Not implemented"));
  }
...
class PostgresClient extends BaseClient {
  ...
  async execute(query, args) {
    return await this.conn.unsafe(query, args);
  }
  async close() {
    return await this.conn.end();
  }
  async constraints() {
    return await this.execute(getPostgresConstraintsQuery(this.quotedSchemas));
  }
  async tableComments() {
    return await this.execute(getPostgresTableCommentsQuery(this.quotedSchemas));
  }
  async columnComments() {
    return await this.execute(getPostgresColumnCommentsQuery(this.quotedSchemas));
  }
  async enums() {
    return await this.execute(getPostgresEnumsQuery(this.quotedSchemas));
  }
}
...
class MySQLClient extends BaseClient {
  ...
  async constraints() {
    return await this.execute(getMySQLConstraintsQuery(this.quotedSchemas));
  }
  async tableComments() {
    return await this.execute(getMySQLTableCommentsQuery(this.quotedSchemas));
  }
  async columnComments() {
    return await this.execute(getMySQLColumnCommentsQuery(this.quotedSchemas));
  }
  enums() {
...
class LibSQLClient extends BaseClient {
  ...
  async constraints() {
    const pk_and_unique = await this.execute(`
      SELECT
        t.schema AS table_schema,
        t.name AS table_name,
        t.schema AS schema,
...
async function getDbSchema(client) {
  ...
  const [tableRows, columnRows, constraintRows, tableCommentRows, columnCommentRows, enumRows] = await Promise.all([
    client.tables(),
    client.columns(),
    client.constraints(),
  ...
  for (const cc of columnCommentRows){
    columnCommentMap.set(`${cc.table_schema}.${cc.table_name}.${cc.name}`, cc.comment);
  }
  // -----------------------------------------------------------------------
  //  Columns
  // -----------------------------------------------------------------------
  for (const c of columnRows){
    ...
    const comment = columnCommentMap.get(`${c.table_schema}.${c.table_name}.${c.name}`);
    table.columns.push({
      name: c.name,
      type: c.type,
  ...
  for (const constraintColumn of constraintRows){
    ...
    if (constraint.type === "FOREIGN KEY" && constraintColumn.type === "FOREIGN KEY") {
      constraint.referenced_table.columns.push(constraintColumn.referenced_column_name);
    }
  }
  // -----------------------------------------------------------------------
  //  DB Schema
...
export async function getDbSchemaImplementation(input, config, _context) {
...
export async function executeSqlImplementation(input, config, _context) {
  const client = createDBClient(config);
  try {
...
const BulkInsertInputSchema = z.object({
  ...
  table: z.string().describe("The table to insert into. It will be created if it does not exist. Use the 'temp_' prefix if the table should be deleted later."),
  columns: z.array(z.string()).optional().describe("Subset of columns to read from the CSV file. If not provided, all columns are used."),
  types: z.array(z.string()).optional().describe("Types of columns, when the table has to be created. If not provided, TEXT is used for all columns."),
  renames: z.array(z.object({
    from: z.string(),
    to: z.string()
  })).optional().describe("Renames columns using a mapper. Columns not in the mapper are left as-is."),
...
export async function bulkInsertImplementation(input, config, _context, supabaseClient) {
  const client = createDBClient(config);
  ...
  if (input.types && input.types.length !== source.length) {
    throw new Error(`Number of types in 'types' must match the number of columns in 'columns' or the number of columns in the CSV file`);
  }
  const target = [];
  for (const col of input.renames?.map((r)=>r.from) || []){
    if (!csvColumns.includes(col)) {
      throw new Error(`From column ${col} declared in 'renames' not found in CSV`);
    }
  }
  for (const col of source){
    ...
    const targetCol = input.renames?.find((r)=>r.from === col)?.to || client.sanitizeIdentifier(col);
  ...
  const table = [
    input.schema,
    input.table
  ].filter(Boolean).map((s)=>client.quoteIdentifier(s)).join(".");
  const createQuery = `
    CREATE TABLE IF NOT EXISTS ${table} (
      ${target.map((col, i)=>`${client.quoteIdentifier(col)} ${input.types?.[i] || "TEXT"}`).join(",\n      ")}
    );
  `;
  ...
  const insertQuery = `
    ...
    VALUES
      ${csv.map((_row)=>"(" + source.map((_col)=>"?").join(", ") + ")").join(",\n      ")}
    ;
  `;
  try {
...
export async function selectAsCsvImplementation(input, config, context, supabaseClient) {
  const client = createDBClient(config);
  try {
    ...
    return {
      file_uri: await uploadToStorage(supabaseClient, context.organization.id, file, input.file_name),
      columns,
...
export const GetDbSchemaTool = {
  ...
  type: "sql",
  name: "getDbSchema",
  description: "Get database schema information including tables, columns, constraints, and enums.",
...
export const SelectAsCsvTool = {
  provider: "local",
  type: "sql",
  name: "selectAsCsv",
  description: "Select data from a SQL database and return it as a CSV file URI.",
...
const SampleTableRowsInputSchema = z.object({
  schemas: z.array(z.string()).optional().describe("Optional: schema names to include."),
  limit: z.number().min(1).max(10).default(3).describe("Number of rows to sample from each table (default: 3, max: 10).")
});
...
export async function sampleTableRowsImplementation(input, config, _context) {
  const client = createDBClient(config);
  client.setSchemas(input.schemas);
  try {
    ...
    for (const table of tableRows){
      const fullTableName = [
        table.schema,
        table.name
      ].map((s)=>client.quoteIdentifier(s)).join(".");
...
export const SampleTableRowsTool = {
  provider: "local",
  type: "sql",
  name: "sampleTableRows",
  description: "Sample N rows from each table in the database to preview actual data.",
