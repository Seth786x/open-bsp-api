...
import { fetchMedia, MAX_STORAGE_UPLOAD_SIZE, uploadToStorage } from "../_shared/media.ts";
 ...
 */ async function buildOrgAddressMap(client, addresses) {
  const { data } = await client.from("organizations_addresses").select().in("address", addresses).eq("status", "connected").eq("service", "whatsapp").order("created_at", {
    ascending: false
  }).throwOnError();
 ...
 */ function collectOrgAddresses(payload) {
 ...
 */ async function validateWebhookSignature(request, body) {
  ...
  if (ids.length !== secrets.length) {
    log.warn("META_APP_ID and META_APP_SECRET environment variables must have the same number of elements, separated by '|'");
    return false;
  }
  let idIndex = 0;
  ...
  if (appId) {
    ...
    if (idIndex === -1) {
      log.warn(`Could not find app_id '${appId}' in META_APP_ID environment variable`);
      return false;
    }
  }
  ...
  try {
    ...
    log.error("Error validating webhook signature", error instanceof Error ? error.message : String(error));
    return false;
  }
}
async function downloadMediaItem({ organization_id, access_token, message, client }) {
  ...
  const response = await fetch(`https://graph.facebook.com/${API_VERSION}/${media_id}`, {
    headers: {
  ...
  const mediaMetadata = await response.json();
...
async function webhookMessageToIncomingMessage(message, organization_id, client) {
  let re_message_id;
  let forwarded;
    ...
    default:
      {
        ...
        log.warn(`Message type "${message.type}" cannot be converted to IncomingMessage`, message);
      }
  }
}
/**
 ...
 */ function extractEditedText(message) {
  switch(message.type){
    ...
    case "image":
      return message.image.caption ? whatsappToMarkdown(message.image.caption) : "";
    case "video":
      return message.video.caption ? whatsappToMarkdown(message.video.caption) : "";
    case "document":
      return message.document.caption ? whatsappToMarkdown(message.document.caption) : "";
    default:
      return undefined;
  }
}
async function processMessage(request) {
  ...
  for (const entry of payload.entry){
    ...
    for (const { value, field } of entry.changes){
      ...
      if (field === "account_update") {
        ...
        const { data: address } = await client.from("organizations_addresses").select().eq("extra->>waba_id", value.waba_info?.waba_id).order("created_at", {
          ascending: false
        ...
        if (value.event === "PARTNER_REMOVED") {
          log.info("Partner removed, disconnecting organization address", value);
          await client.from("organizations_addresses").update({
            status: "disconnected"
          }).eq("organization_id", address.organization_id).eq("extra->>waba_id", value.waba_info?.waba_id).throwOnError();
        }
      }
      ...
      if ((field === "messages" || field === "history") && "messages" in value) {
        for (const webhookMessage of value.messages){
          ...
          const content = await webhookMessageToIncomingMessage(webhookMessage, organization_id, client);
          // --- START OF GATEKEEPER ---
          // Only trigger n8n if this is a REAL incoming message
          if (content && content.direction === "incoming") {
            const { data: webhook } = await client.from("webhooks").select("url").eq("organization_id", organization_id).single();
            if (webhook?.url) {
              // Fire and forget: send to n8n only for actual inbound messages
              fetch(webhook.url, {
                method: "POST",
                headers: {
              ...
              }).catch((err)=>log.error("Failed to trigger n8n", err));
            }
          }
          // --- END OF GATEKEEPER ---
          if (!content) {
      ...
      if (field === "messages" && "statuses" in value) {
        for (const status of value.statuses){
          statuses.push({
            ...
            content: {},
            status: {
              [status.status]: new Date(parseInt(status.timestamp) * 1000).toISOString(),
      ...
      if (field === "history" && "history" in value) {
        for (const history of value.history){
          if ("threads" in history) {
            ...
            const msgCount = history.threads.reduce((acc, thread)=>acc + thread.messages.length, 0);
            ...
            for (const thread of history.threads){
              for (const webhookMessage of thread.messages){
                ...
                const contact_address = isEcho ? webhookMessage.to : webhookMessage.from;
                if (webhookMessage.type === "errors") {
                  errors.push(...webhookMessage.errors);
                  continue;
                }
                ...
                const originalStatus = webhookMessage.history_context.status.toLowerCase();
                const status = historyStatusMap[originalStatus] || originalStatus;
                const message = isEcho ? {
                  ...
                  content: content,
                  status: {
                    [status]: new Date(webhookMessage.timestamp * 1000).toISOString()
                  },
                  timestamp: new Date(webhookMessage.timestamp * 1000).toISOString()
                } : {
                  organization_id,
      ...
      if (errors.length > 0) {
        const errorCounts = errors.reduce((acc, curr)=>{
  ...
  const downloadMediaPromise = Promise.all(messages.map(async (message)=>{
  ...
  if (contacts_addresses.length > 0) {
    ...
    const dedupedContactsAddresses = Array.from(new Map(contacts_addresses.map((ca)=>[
        `${ca.organization_id}|${ca.address}|${ca.service}`,
        ca
      ])).values());
    const { error: contactsError } = await client.from("contacts_addresses").upsert(dedupedContactsAddresses);
    if (contactsError) {
  ...
  const upsertBatch = async (label, rows)=>{
    if (rows.length === 0) return;
    const { error } = await client.from("messages").upsert(rows, {
  ...
  for (const { original_message_id, text, timestamp } of edits){
    ...
    }).eq("external_id", original_message_id).throwOnError();
  }
