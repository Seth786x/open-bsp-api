...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    console.log("DEBUG: test-outbound intercepted payload:", {
      phone_number_id,
      to,
      message
    });
    // 3. Locate Target Configuration via adminClient (bypasses RLS)
    const { data: account, error: dbError } = await adminClient.from("organizations_addresses").select("extra").eq("address", phone_number_id).single();
    if (dbError || !account) {
      return new Response(JSON.stringify({
        error: "Access Denied: Configuration Verification Failed"
      }), {
        status: 403,
        headers: {
          ...corsHeaders,
