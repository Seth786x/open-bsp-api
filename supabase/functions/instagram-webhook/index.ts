 ...
 */ async function buildOrgAddressMap(client, addresses) {
  ...
  const { data } = await client.from("organizations_addresses").select().in("address", addresses).eq("status", "connected").eq("service", "instagram").order("created_at", {
    ascending: false
  }).throwOnError();
  const map = new Map();
 ...
 */ async function downloadMediaItem({ organization_id, message, client }) {
 ...
 */ async function fetchProfile(igsid, accessToken) {
  if (!accessToken) {
    log.warn(`No Instagram access token, cannot fetch profile for ${igsid}`);
    return undefined;
  }
  try {
    const response = await fetch(`https://graph.instagram.com/${API_VERSION}/${igsid}?fields=username,name,biography,profile_picture_url&access_token=${accessToken}`);
    if (!response.ok) {
      const errorBody = await response.text().catch(()=>"");
      log.warn(`Failed to fetch IG profile for ${igsid}: ${response.status} — ${errorBody}`);
      return undefined;
    }
 ...
 */ function instagramMessageToContent(msg) {
  const base = {
    version: "1",
    ...msg.reply_to?.mid && {
      re_message_id: msg.reply_to.mid
    },
    ...msg.reply_to?.story && {
      re_story: msg.reply_to.story
    },
    ...msg.referral && {
...
async function processMessage(request) {
  ...
  const orgAddressMap = await buildOrgAddressMap(client, collectOrgAddresses(payload));
  const contactNeeds = new Map();
  for (const entry of payload.entry){
    for (const event of extractEvents(entry)){
  ...
  if (contactNeeds.size > 0) {
    const orgIds = Array.from(new Set(Array.from(contactNeeds.values()).map((c)=>c.organization_id)));
    const igsids = Array.from(new Set(Array.from(contactNeeds.values()).map((c)=>c.igsid)));
    const { data } = await client.from("contacts_addresses").select("organization_id, address, service, extra").in("organization_id", orgIds).eq("service", "instagram").in("address", igsids).throwOnError();
    for (const row of data){
      // Narrow union via the discriminant.
      if (row.service !== "instagram") continue;
      cache.set(`${row.organization_id}|${row.address}`, row.extra ?? {});
    }
  }
  ...
  for (const c of contactNeeds.values()){
    ...
    const fetchedAt = cached?.name_fetched_at ? Date.parse(cached.name_fetched_at) : 0;
  ...
  const fetchedProfiles = await Promise.all(fetchTasks.map(async (c)=>({
      key: `${c.organization_id}|${c.igsid}`,
      contact: c,
  ...
  for (const entry of payload.entry){
    for (const event of extractEvents(entry)){
      ...
      if (event.message?.is_self) {
        log.info("Skipping is_self test message", {
      ...
      if (event.message_edit) {
        messages.push({
          ...
          content: {
            ...
            data: {
              num_edit: event.message_edit.num_edit
            }
          },
          status: {
            edited: timestamp
  ...
  const downloadMediaPromise = Promise.all(messages.map(async (message)=>{
    ...
    try {
      ...
      log.warn("Failed to download IG media, preserving message with original reference", {
        error: error instanceof Error ? error.message : String(error),
  ...
  if (contacts_addresses.length > 0) {
    ...
    const { error: contactsError } = await client.from("contacts_addresses").upsert(contacts_addresses);
    if (contactsError) {
  ...
  if (allMessages.length > 0) {
    const { error: messagesError } = await client.from("messages").upsert(allMessages, {
